...
class DummyTest(LucteriosTest):
    ...
    def test_formating_general(self):
        ...
        self.assertEqual(format_to_string("abc", None, "{0}"), "abc", "check string simple +")
        self.assertEqual(format_to_string(["abc", "uvw", "xyz"], None, None), "abc{[br/]}uvw{[br/]}xyz", "check string multiple")
        self.assertEqual(format_to_string("abc", None, "{[i]}{[b]}{0}{[/b]}{[/i]}"), "{[i]}{[b]}abc{[/b]}{[/i]}", "check string formated")
        self.assertEqual(format_to_string(["abc", "uvw", "xyz"], None, "{0} : {1} --> {2}"), "abc : uvw --> xyz", "check string splited")
        self.assertEqual(format_to_string([65.4, 456.04, 894730.124], "N2", "{0} : {1} --> {2}"), "65,40 : 456,04 --> 894 730,12", "check float splited")
        self.assertEqual(format_to_string({"value": "abc", "format": "{[b]}{0}{[/b]}"}, None, "{[i]}{0}{[/i]}"), "{[i]}{[b]}abc{[/b]}{[/i]}", "check string riched formated")

        self.assertEqual(format_to_string(1234.56, None, "{[i]}{0}{[/i]};{[b]}{0}{[/b]}"), "{[i]}1234.56{[/i]}", "check num positive")
        self.assertEqual(format_to_string(-1234.56, None, "{[i]}{0}{[/i]};{[b]}{0}{[/b]}"), "{[b]}1234.56{[/b]}", "check num negative")

        self.assertEqual(format_to_string(0.000001, 'C2EUR', '{0};A'), "A", "currency mode 0 null")
        self.assertEqual(format_to_string(-0.000001, 'C2EUR', '{0};A'), "A", "currency mode 0 null")
        self.assertEqual(format_to_string(-0.000001, 'C2EUR', '{0};A;B'), "B", "currency mode 0 null")
        self.assertEqual(format_to_string(0.000001, 'C2EUR', '{0};A;B'), "B", "currency mode 0 null")

        self.assertEqual(format_to_string(0, {'0': 'aaa', '1': 'bbb', '2': 'ccc'}, "{0}"), "aaa", "check select 0")
        self.assertEqual(format_to_string(1, {'0': 'aaa', '1': 'bbb', '2': 'ccc'}, "{0}"), "bbb", "check select 1")
        self.assertEqual(format_to_string(2, {'0': 'aaa', '1': 'bbb', '2': 'ccc'}, "{0}"), "ccc", "check select 2")
        self.assertEqual(format_to_string(3, {'0': 'aaa', '1': 'bbb', '2': 'ccc'}, "{0}"), "3", "check select 3")

    def test_formating_fr(self):
        set_locale_lang('fr')
        activate('fr')

        self.assertEqual(format_to_string(1234.56, "N3", "{[i]}{0}{[/i]};{[b]}{0}{[/b]}"), "{[i]}1 234,560{[/i]}", "check num positive formated")
        self.assertEqual(format_to_string(-1234.56, "N3", "{[i]}{0}{[/i]};{[b]}{0}{[/b]}"), "{[b]}1 234,560{[/b]}", "check num negative formated")
        self.assertEqual(format_to_string(1234.56, "N0", "{0}"), "1 235", "check int no-formated")
        ...
        self.assertEqual(format_to_string(1234.56, "C2EUR", "{0}"), "1 234,56 €", "check currency positive no-formated")
        self.assertEqual(format_to_string(-1234.56, "C2EUR", "{0}"), "-1 234,56 €", "check currency negative no-formated")

        self.assertEqual(format_to_string(1234.56, 'C2EUR', '{0};'), "1 234,56 €", "currency mode 0 +")
        self.assertEqual(format_to_string(1234.56, 'C2EUR', 'Crédit {0};Débit {0}'), "Crédit 1 234,56 €", "currency mode 1 +")
        self.assertEqual(format_to_string(1234.56, 'C2EUR', '{[font color="green"]}Crédit {0}{[/font]};{[font color="blue"]}Débit {0}{[/font]}'), "{[font color=\"green\"]}Crédit 1 234,56 €{[/font]}", "currency mode 2 +")
        self.assertEqual(format_to_string(1234.56, 'N2', '{0}'), "1 234,56", "currency mode 3 +")
        self.assertEqual(format_to_string(1234.56, 'C2EUR', '{0};{0}'), "1 234,56 €", "currency mode 4 +")
        self.assertEqual(format_to_string(1234.56, 'C2EUR', '{0}'), "1 234,56 €", "currency mode 5 +")
        self.assertEqual(format_to_string(1234.56, 'C2EUR', '{[font color="green"]}{0}{[/font]};{[font color="blue"]}{0}{[/font]}'), "{[font color=\"green\"]}1 234,56 €{[/font]}", "currency mode 6 +")

        self.assertEqual(format_to_string(-1234.56, 'C2EUR', '{0};'), "", "currency mode 0 -")
        self.assertEqual(format_to_string(-1234.56, 'C2EUR', 'Crédit {0};Débit {0}'), "Débit 1 234,56 €", "currency mode 1 -")
        self.assertEqual(format_to_string(-1234.56, 'C2EUR', '{[font color="green"]}Crédit {0}{[/font]};{[font color="blue"]}Débit {0}{[/font]}'), "{[font color=\"blue\"]}Débit 1 234,56 €{[/font]}", "currency mode 2 -")
        self.assertEqual(format_to_string(-1234.56, 'N2', '{0}'), "-1 234,56", "currency mode 3 -")
        self.assertEqual(format_to_string(-1234.56, 'C2EUR', '{0};{0}'), "1 234,56 €", "currency mode 4 -")
        self.assertEqual(format_to_string(-1234.56, 'C2EUR', '{0}'), "-1 234,56 €", "currency mode 5 -")
        self.assertEqual(format_to_string(-1234.56, 'C2EUR', '{[font color="green"]}{0}{[/font]};{[font color="blue"]}{0}{[/font]}'), "{[font color=\"blue\"]}1 234,56 €{[/font]}", "currency mode 6 -")

        self.assertEqual(format_to_string("2017-04-23", "D", "{0}"), "23 avril 2017", "check date")
        self.assertEqual(format_to_string("12:54:25.014", "T", "{0}"), "12:54", "check time")
        self.assertEqual(format_to_string("2017-04-23T12:54:25.014", "H", "{0}"), "dimanche 23 avril 2017 à 12:54", "check date time")

        self.assertEqual(format_to_string(True, "B", "{0}"), "Oui", "check bool true")
        self.assertEqual(format_to_string(False, "B", "{0}"), "Non", "check bool false")
    ...
    def test_formating_en(self):
        ...
        self.assertEqual(format_to_string("12:54:25.014", "T", "{0}"), "12:54", "check time")
        self.assertEqual(format_to_string("2017-04-23T12:54:25.014", "H", "{0}"), "Sunday, April 23, 2017 at 12:54", "check date time")

        self.assertEqual(format_to_string(True, "B", "{0}"), "Yes", "check bool true")
        self.assertEqual(format_to_string(False, "B", "{0}"), "No", "check bool false")
