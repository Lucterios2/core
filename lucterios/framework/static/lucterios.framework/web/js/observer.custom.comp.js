...
var compLabelForm = compGeneric.extend({
    ...
    initial : function (component) {
        ...
        this.label = component.getTextFromXmlNode().convertLuctoriosFormatToHtml();
        this.tag = 'span';
    },
    ...
    initial : function (component) {
        ...
        if (acts.length > 0) {
            ...
            if (act.length > 0) {
                ...
                this.action.initialize(this.owner, Singleton().Factory(), act[0]);
                this.action.setCheckNull(false);
            }
        }
        this.java_script_content = decodeURIComponent(component.getCDataOfFirstTag('JavaScript').replace(/\+/g, " "));
    },
...
var compFloat = compAbstractEvent.extend({
    ...
    initial : function (component) {
        this._super(component);
        var text_value = '', nb_dec = component.getXMLAttributInt('prec', 0), dec_i;
...
var editorHypertext = Class.extend({
    ...
    init : function (aOwnerId, aName, aWith_hypertext, aSub_menu_xml) {
        ...
        for (isub_menu = 0; isub_menu < aSub_menu_xml.length; isub_menu++) {
            ...
            this.sub_menus.push([ sub_menu_name, sub_menu_value ]);
            this.submenus[sub_menu_value] = {
                name : sub_menu_name
            };
        }
    },
...
var compMemo = compAbstractEvent.extend({
    ...
    initial : function (component) {
        ...
        this.editor = new editorHypertext(this.owner.getId(), this.name, component.getXMLAttributInt('with_hypertext', 0), component.getElementsByTagName("SUBMENU"));
        this.tag = 'textarea';
    },
...
var compXML = compAbstractEvent.extend({
    ...
    initial : function (component) {
        this._super(component);
        this.value = component.getTextFromXmlNode();
        this.editor = new editorHypertext(this.owner.getId(), this.name, 1, component.getElementsByTagName("SUBMENU"));
        this.tag = 'textarea';
    },
...
var compDate = compAbstractEvent.extend({
    ...
    initial : function (component) {
        ...
        if (this.value === '') {
            var today = new Date();
            this.value = "{0}-{1}-{2}".format(today.getFullYear(), (today.getMonth() + 1).toString().lpad('0', 2), today.getDate().toString().lpad('0', 2));
        }
        this.tag = 'input';
    },
...
var compDateTime = compAbstractEvent.extend({
    ...
    initial : function (component) {
        ...
        if (this.value === '') {
            var today = new Date();
            this.value = "{0}-{1}-{2} {3}:{4}".format(today.getFullYear(), (today.getMonth() + 1).toString().lpad('0', 2), today.getDate().toString().lpad('0', 2), today.getHours().toString().lpad(
                    '0', 2), today.getMinutes().toString().lpad('0', 2));
        }
        this.tag = 'input';
    },
...
var compSelectCase = Class.extend({
    ...
    getHtml : function (value) {
        ...
        if (((value !== null) && (value === this.id)) || ((value === null) && (this.checked === '1'))) {
            html += ' selected';
        }
...
var compSelect = compAbstractEvent.extend({
    ...
    initial : function (component) {
        ...
        if ((component.firstChild !== null) && (component.firstChild.nodeValue !== null)) {
...
var compUpload = compGeneric.extend({
    ...
    getEncodeFile : function (fileToSend) {
        ...
        if (fileToSend.size > this.maxsize) {
            ...
            throw new LucteriosException(IMPORTANT, Singleton().getTranslate("Upload impossible<br/>The file must be less than {0}Mo ").format(size));
        }
        if (this.httpFile) {
...
var compdownload = compGeneric.extend({
    ...
    getHtml : function () {
        ...
        return "<label>" + this.file_name + "</label>" + this.getBuildHtml(args, false, false) + Singleton().getTranslate("Save as...") + "</button>";
    },
