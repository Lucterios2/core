...
var editorHypertext = Class.extend({
	...
	init : function(aOwnerId, aName, aWith_hypertext, aSub_menu_xml) {
		...
		for (isub_menu = 0; isub_menu < aSub_menu_xml.length; isub_menu += 1) {
			...
			this.sub_menus[this.sub_menus.length] = [ sub_menu_name, sub_menu_value ];
		}
	},
	getGUIComp : function() {
	...
	getHtml : function(attribut, value) {
		...
		html += '<ul id="menu_{0}" class="contextMenu">'.format(this.name);
		for (imenu = 0; imenu < this.sub_menus.length; imenu += 1) {
			sub_menu = this.sub_menus[imenu];
			html += '<li><a href="#menu_{0}">{1}</a></li>'.format(imenu, sub_menu[0]);
		}
		html += '</ul>';
	...
	addEditorAction : function() {
		var menu_name = "menu_{0}".format(this.name), area_name = "textarea[name='{0}']".format(this.name), self = this;
		if (this.sub_menus.length > 0) {
			$(area_name).contextMenu({
				menu : menu_name
...
var CompMemo = compAbstractEvent.extend({
	...
	initial : function(component) {
		...
		this.editor = new editorHypertext(this.owner.getId(), this.name, component.getXMLAttributInt('with_hypertext', 0), component
