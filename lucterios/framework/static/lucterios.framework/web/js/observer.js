...
var ObserverAbstract = Class
		.extend({
			...
			getContentText : function () {
				...
				for (node_idx = 0; node_idx < this.mDomXmlContent.childNodes.length; node_idx += 1) {
			...
			fillContext : function () {
				...
				var xmlcontexts = this.mDomXmlContent.getElementsByTagName("CONTEXT"), xmlparams, index;
			...
			setContent : function (aDomXmlContent) {
				...
				if (this.mDomXmlContent !== null) {
					this.fillContext();
					var xml_item_closes = this.mDomXmlContent.getElementsByTagName("CLOSE_ACTION"), xml_items, action_idx;
					if (xml_item_closes.length > 0) {
						xml_items = xml_item_closes[0]
								.getElementsByTagName("ACTION");
						for (action_idx = 0; (this.mCloseAction === null)
								&& (action_idx < xml_items.length); action_idx += 1) {
...
var ObserverGUI = ObserverAbstract.extend({
	...
	buildButtons : function () {
		...
		if (this.mActions !== null) {
			...
			for (index = 0; index < xml_actions.length; index += 1) {
