...
var Menu = Class
		.extend({
			...
			init : function(menu, level, owner) {
				...
				if (this.txt !== undefined && this.txt !== null
						&& this.txt !== "") {
				...
				for (issMen = 0; issMen < menu.childNodes.length; issMen += 1) {
					...
					if (ssMenu.tagName === "MENU") {
						...
						if ((((menu_item.icon !== '') || G_With_Extra_Menu) && (menu_item.action !== null))
					...
					if (this.level === 0) {
						if (this.icon !== "") {
							this.rootHtml += '<img src="{0}" />'
									.format(singleton().Transport().getIconUrl(
											this.icon));
						}
...
var ObserverMenu = ObserverAbstract
		.extend({
			...
			setActive : function(aIsActive) {
				...
				if (this.aside_menu) {
					for (submenu_idx = 0; submenu_idx < this.aside_menu.submenu.length; submenu_idx += 1) {
						...
						$("#title_{0}".format(current_menu.txt)).prop(
								"disabled", !aIsActive);
					}
				}
			...
			show : function(aTitle, aGUIType) {
				...
				this.buildAsideMenu();
				html = '<div id="menuContainer">' + createTab(roothtml, html)
						+ '</div>';
				...
				if (this.aside_menu !== null) {
					...
					$("#mainMenu > div.vsplitter").dblclick(
							function() {
								...
								main_menu.find('.splitter_panel').trigger(
										'splitter.resize');
							});
				} else {
					$("#mainMenu").css('display', 'block');
			...
			addMenuButton : function(menu, parent) {
				var html = CONTENT_MENU_BUTTON_TEMPLATE.format(menu.icon,
						menu.txt, menu.help, menu.id), act, bt;
				$("#{0}".format(parent)).append(html);

				act = new ActionInt();
				act.initializeEx(menu, null, menu.txt, menu.extension,
						menu.action);
			...
			addSubMenu : function(menus, parent) {
				...
				for (j = 0; j < menus.length; j += 1) {
					...
					if (m2.action !== null) {
						...
						html = CONTENT_MENU_INSIDE_PANEL_TEMPLATE.format(m2.id,
								m2.icon, m2.txt);
						$("#{0}".format(parent)).append(html);

						if (m2.submenu.length > 0) {
							this.addSubMenu(m2.submenu, "{0} > .panel-body"
									.format(m2.id));
						}
					}
				}
			...
			buildAsideMenu : function() {
				var html = '', active = '', menu_idx, current_menu, submenu_idx;
				$("#mainmenucontent").html('');
				$("#mainsidemenu").html('');
				if (this.mAction === "menu") {
					// menu principal
					for (menu_idx = 0; menu_idx < this.menu_list.length; menu_idx += 1) {
						current_menu = this.menu_list[menu_idx];

						if (current_menu.id === "coremenu") {
							for (submenu_idx = 0; submenu_idx < current_menu.submenu.length; submenu_idx += 1) {
							...
							html = SIDE_MENU_BUTTON_TEMPLATE.format(active,
									current_menu.id, current_menu.icon,
									current_menu.txt);
							$("#mainsidemenu").append(html);

							// sous-menus dans un panel
							html = CONTENT_MENU_MAIN_DIV_TEMPLATE
									.format(current_menu.id);
							$("#mainmenucontent").append(html);

							this.addSubMenu(current_menu.submenu,
									current_menu.id);
						}
					}
				}
