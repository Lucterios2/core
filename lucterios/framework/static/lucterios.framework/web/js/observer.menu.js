...
var ObserverMenu = ObserverAbstract
		.extend({
			...
			buildAsideMenu : function() {
				...
				if (this.aside_menu !== null) {
					html = '<div id="asideMenu">';
					for (submenu_idx = 0; submenu_idx < this.aside_menu.submenu.length; submenu_idx++) {
						current_menu = this.aside_menu.submenu[submenu_idx];
						current_menu.obs = new ObserverCustom();
						current_menu.obs.setSource(current_menu.extension,
								current_menu.action);
						current_menu.obs.setContent("<null/>".parseXML());
						current_menu.obs.mID = "aside_{0}"
								.format(submenu_idx + 1);
						html += '<h3 id="title_{0}">{1}</h3>'.format(
								current_menu.obs.getId(), current_menu.txt);
						html += '<div id="{0}"></div>'.format(current_menu.obs
								.getId());
					}
					html += '</div>';
					$("#mainMenu").append(html);
					...
					for (submenu_idx = 0; submenu_idx < this.aside_menu.submenu.length; submenu_idx++) {
						$("#aside_{0}".format(submenu_idx + 1))[0].style.height = size
								+ 'px';
					}
				}
				return html;
