		...
		.extend({
			...
			showGUI : function(isModal) {
				...
				for (i = 0; i < list.length; i += 1) {
				    bt_html += BUTTON_TEMPLATE.format(this.mId, i, list[i].text);
				}
				html = MODAL_TEMPLATE.format(this.mId, this.mTitle, this.mHtmlBody, bt_html);
				$("body").append(html);
				for (i = 0; i < list.length; i += 1) {
				    $('#BT_{0}_{1}'.format(this.mId, i)).on('click', list[i].click);
				}
				$('#' + this.mId).modal();
			},
			...
			buildFinal : function () {
				...
				for (iAct = 0; iAct < this.mButtons.length; iAct += 1) {
					if (this.mButtons[iAct].getIcon() !== '') {
						...
						btn = parent.find("div > div > button:eq({0}) > span".format(iAct));
						btn.find('img').remove();
...
function createTable(compArray) {
	...
	for (row_idx = 0; row_idx < compArray.length; row_idx += 1) {
		if ((compArray[row_idx] instanceof Array)
				&& (compArray[row_idx].length > 0)) {
			html += '<tr>';
			line = compArray[row_idx];
			for (col_idx = 0; col_idx < line.length; col_idx += 1) {
				if ((line[col_idx] !== undefined) && (line[col_idx] !== null)) {
					...
					html += '<td rowspan="{0}" colspan="{1}" style="{2}">{3}</td>'
							.format(compo.rowspan, compo.colspan, compo.style,
									compo.getHtml());
					for (colspan_idx = 0; colspan_idx < compo.colspan; colspan_idx += 1) {
						line[col_idx + colspan_idx] = null;
						for (rowspan_idx = 0; rowspan_idx < compo.rowspan; rowspan_idx += 1) {
...
function createTab(tabs, tabcontents) {
	...
	html += "<ul>";
	for (t_idx = 0; t_idx < tabs.length; t_idx += 1) {
		html += '<li><a href="#tab_{0}_{1}">{2}</a></li>'.format(tab_Id, t_idx,
				tabs[t_idx]);
	}
	html += "</ul>";
	for (t_idx = 0; t_idx < tabcontents.length; t_idx += 1) {
