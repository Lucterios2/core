...
var CompGridRow = Class
		.extend({
			...
			initial : function (component, grd) {
				...
				this.values = [];
				for (idx_val = 0; idx_val < vals.length; idx_val += 1) {
			...
			getHtml : function () {
				...
				for (idx_val = 0; idx_val < this.values.length; idx_val += 1) {
...
var CompGrid = compGeneric
		.extend({
			...
			initial : function (component) {
				...
				this.gridHeaders = [];
				for (iChild = 0; iChild < heads.length; iChild += 1) {
				...
				this.gridRows = [];
				for (iChild = 0; iChild < rows.length; iChild += 1) {
					row = new CompGridRow();
				...
				if (actions !== null) {
					...
					for (iChild = 0; iChild < acts.length; iChild += 1) {
			...
			addPageSelector : function () {
				...
				if (this.page_max > 1) {
					...
					for (idx = 0; idx < this.page_max; idx += 1) {
			...
			getHtml : function () {
				...
				for (iHead = 0; iHead < this.gridHeaders.length; iHead += 1) {
					html += this.gridHeaders[iHead].getHtml();
				}
				html += '</tr></thead><tbody>';
				// parcours des rows
				for (iRow = 0; iRow < this.gridRows.length; iRow += 1) {
				...
				if (this.buttons.length > 0) {
					...
					for (iBtn = 0; iBtn < this.buttons.length; iBtn += 1) {
			...
			getValueType : function (name) {
				var iCol;
				for (iCol = 0; iCol < this.gridHeaders.length; iCol += 1) {
