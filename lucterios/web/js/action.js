...
var ActionAbstract = Class.extend({
    ...
    initialize: function(aOwner, aFactory, aXml) {
        var xmlparams, index;
        this.initializeEx(aOwner, aFactory, aXml.getTextFromXmlNode(), aXml.getAttribute("extension"), aXml.getAttribute("action"));
        ...
        for (index = 0; index < xmlparams.length; index++) {
            this.mExtraParam.put(xmlparams[index].getAttribute("name"), xmlparams[index].childNodes.length>0?xmlparams[index].childNodes[0].nodeValue:'');
        }
    },
    ...
    getParameters: function() {
        ...
        if ((this.mExtraParam !== null) && (this.mExtraParam.size() > 0)) {
        	if (param === null) {
        		param = new HashMap();
        	}
