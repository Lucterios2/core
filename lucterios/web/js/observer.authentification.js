...
var ApplicationDescription = Class.extend({
    ...
    init: function(aTitle, aCopyRigth, aLogoName, aAppliVersion, aServerVersion) {
        this.mTitle = aTitle;
    ...
    getHTML: function(aComplement) {
        ...
        resValue += "<hr><center><h1>" + this.mTitle + "</h1></center>" +
            "<table width='100%'>" +
            "<tr><td><center>" + Singleton().getTranslate("Version") + "</center></td><td><center>" + this.mApplisVersion + "</center></td></tr>" +
            "<tr><td><center>" + Singleton().getTranslate("Server") + "</center></td><td><center>" + this.mServerVersion + "</center></td></tr>" +
            "<tr><td><center>" + Singleton().getTranslate("AJAX Client") + "</center></td><td><center>" + G_Version + "</center></td></tr>" +
            "<tr><td><center>" + Singleton().getTranslate("Browser") + "</center></td><td><center><font size='-1'>" + navigator.userAgent.replace(') ', ')<br>') + "</font></center></td></tr>" +
            "<tr><td><center>" + Singleton().getTranslate("Connection") + "</center></td><td><center><font size='-1'>" + this.getConnectUser() + '<br>' + get_serverurl() + "</font></center></td></tr>" +
            "<tr><td colspan='2'><center><font size='-1'><i>" + this.mCopyRigth + "</i></font></center></td></tr>" +
            "</table>" + "<hr>";
    ...
    getText: function(aComplement) {
        var text_html = this.getHTML(aComplement),
            pos1 = 1,
            pos2 = 1;
        while ((pos1 > 0) && (pos2 > 0)) {
        ...
        text_html = text_html.replace(/(<center>|<\/center>|<table width='100%'>|<\/table>|<tr><td>|<tr><td colspan='2'>|<font size='-1'>|<\/font>|<i>|<\/i>)/g, "");
...
var ObserverAuthentification = ObserverAbstract.extend({
    ...
    setContent: function(aDomXmlContent) {
        ...
        this.acts[0].callback = $.proxy(function(aParams) {
            if (this.mGUI !== null) {
                var login = aParams.get(AUTH_PARAM_NAME[0]),
                    pass = aParams.get(AUTH_PARAM_NAME[1]);
    ...
    show: function(aTitle, aGUIType) {
        ...
        var cdate = this.mDomXmlContent.getTextFromXmlNode().trim(),
            xml_connection,
            desc,
            xml_params,
            param_idx;
        if (cdate !== "OK") {
            ...
            desc = new ApplicationDescription(xml_connection.getCDataOfFirstTag("TITLE"),
