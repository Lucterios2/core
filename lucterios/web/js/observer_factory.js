...
var ObserverFactoryAbstract = Class.extend({
    ...
    setAuthentification: function(aLogin, aPassWord) {
        ...
        if ((aLogin === null)) {
       	...
       	aut = this.callAction(AUTH_PARAM_NAME[4], "authentification", param, null);
...
var ObserverFactoryImpl = ObserverFactoryAbstract.extend({
    ...
    convertParameters: function(aExtension, aAction, aParam) {
        var result = new HashMap(),
            self = this,
            val;
        this.m_XMLParameters = "<REQUETE extension='" + aExtension + "' action='" + aAction + "'>";
        aParam.keys().forEach(function(key) {
            val = aParam.get(key);
            if (typeof val === 'string') {
                self.m_XMLParameters = self.m_XMLParameters + "<PARAM name='" + key + "'><![CDATA[" + val + "]]></PARAM>";
            } else {
                result.put(key, val);
                if (val instanceof Blob) {
                    self.m_XMLParameters = self.m_XMLParameters + "<PARAM name='" + key + "'><![CDATA[" + val.name + "]]></PARAM>";
                }
            }
        });
	...
	transfertFromServer: function(aExtension, aAction, aParam) {
		return this.mTransport.transfertXMLFromServer(this.convertParameters(aExtension, aAction, aParam));
	},

    callAction: function(aExtension, aAction, aParam, aObserver) {
        var res_obs = null,
            xml_text = this.transfertFromServer(aExtension, aAction, aParam),
            reps = xml_text.parseXML(),
			rep = null, idx,
            observer_name, titles,
            source_extension, source_action,
            errorMsg;
        post_log("==>  reponse :" + xml_text);
		for (idx = 0; (reps !== null) && (rep === null) && (idx < reps.childNodes.length); idx++) { 
        ...
        if (rep !== null) {
            ...
            if (this.mObserverList.hasOwnProperty(observer_name)) {
                ...
                if (aObserver === null) {
                    res_obs = this.factoryObserver(observer_name, this.m_XMLParameters, xml_text);
                    ...
                    if ((res_obs.getObserverName() !== observer_name) || (source_action === "") || (source_extension === "")) {
                        if ((observer_name === "CORE.Auth") || (observer_name === "CORE.Exception")) {
                            ...
                            throw new LucteriosException(GRAVE, "Erreur de parsing xml (refresh)" + aObserver.getObserverName() + " -> " + observer_name, this.m_XMLParameters, xml_text);
                        }
                    }
                }
                ...
                throw new LucteriosException(IMPORTANT, "Observeur '" + observer_name + "' inconnu{[newline]}Veuillez utiliser le client Java.", this.m_XMLParameters, xml_text);
            }
        } else {
            errorMsg = "**";
            if (reps === null) {
            ...
            throw new LucteriosException(GRAVE, "Erreur de parsing xml", this.m_XMLParameters, xml_text + "\nMsg:" + errorMsg);
        }
        return res_obs;
    }
});
