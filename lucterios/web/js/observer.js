...
var ObserverAbstract = Class.extend({
    ...
    fillContext: function() {
        ...
        var xmlcontexts = this.mDomXmlContent.getElementsByTagName("CONTEXT"),
            xmlparams,
            index;
        if (xmlcontexts.length > 0) {
            xmlparams = xmlcontexts[0].getElementsByTagName("PARAM");
            for (index = 0; index < xmlparams.length; index++) {
                this.mContext.put(xmlparams[index].getAttribute("name"), xmlparams[index].childNodes.length>0?xmlparams[index].childNodes[0].nodeValue:'');
            }
        }
    },
    ...
    setContent: function(aDomXmlContent) {
        ...
        if (this.mDomXmlContent !== null) {
            this.fillContext();
            var xml_item_closes = this.mDomXmlContent.getElementsByTagName("CLOSE_ACTION"),
                xml_items,
                action_idx;
            if (xml_item_closes.length > 0) {
                xml_items = xml_item_closes[0].getElementsByTagName("ACTION");
                for (action_idx = 0;
                    (this.mCloseAction === null) && (action_idx < xml_items.length); action_idx++) {
                    this.mCloseAction = Singleton().CreateAction();
                    this.mCloseAction.initialize(this, Singleton().Factory(), xml_items[action_idx]);
            ...
            this.mRefresh.initializeEx(this, Singleton().Factory(), this.getTitle(),
                this.getSourceExtension(), this.getSourceAction());
    ...
    buildButtons: function() {
        ...
        if (this.mActions !== null) {
            ...
            for (index = 0; index < xml_actions.length; index++) {
                ...
                new_act.initialize(this, Singleton().Factory(), xml_actions[index]);
