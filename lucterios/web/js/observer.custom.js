...
var ObserverCustom = ObserverGUI.extend({
    ...
    show: function(aTitle, aGUIType) {
        ...
        for (comp_idx = 0; comp_idx < this.mCompList.size(); comp_idx++) {
            ...
            this.mCompList.val(comp_idx).getGUIComp().focusin($.proxy(this.savefocusin, this));
        }
    ...
    checkCompoundEmpty: function() {
        ...
        if (cmp !== null) {
            ...
            if (cmp.description.length > 0) {
                msg_text = "Le champ '{0}' est obligatoire!".format(decodeURIComponent(cmp.description.replace(/\+/g, ' ')));
            }
...
var compGeneric = compBasic.extend({
    ...
    isEmpty: function() {
        return (this.needed && ((this.getValue() === null) || (this.getValue() === '')));
    },
