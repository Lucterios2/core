...
test( "CallAction", function() {
	...
	var obs = this.mObserverFactory.callAction("CORE","printmodelReinit", current_params, null);
	...
	equal(obs.getContentText(),'<TITLE>Réinitialisation de modele</TITLE><TEXT type="2"><![CDATA[Etes-vous sure de reinitialiser ce modele?]]></TEXT><ACTIONS><ACTION action="printmodelReinit" extension="CORE" icon="images/ok.png"><![CDATA[Oui]]></ACTION><ACTION icon="images/cancel.png"><![CDATA[Non]]></ACTION></ACTIONS>'
			,"Content");

	equal(obs.getTitle(),"Réinitialisation de modele","Titre");
	
	equal(this.mHttpTransport.XmlParam.size(), 1, 'XmlParam size');
	equal(this.mHttpTransport.XmlParam['WebFile'] , "CORE/printmodelReinit", 'WebFile');
});
...
test( "CallActionWithParam", function() {
	...
	var obs = this.mObserverFactory.callAction("CORE", "printmodelReinit", params, null);
...
test( "CallActionBadXmlReceived", function() {
	...
	try {
		...
		equal(err.toString().substring(0, 86), "2~Erreur de parsing xml#CORE/printmodelReinit?#Error php in truc.inc.php line 123\nMsg:");
	}
});
...
test( "CallActionBadObserver", function() {
	...
	try {
		...
		equal(err.message,"Observeur 'Core.BialogDox' inconnu{[newline]}Veuillez utiliser le client Java.");
		equal(err.toString().substring(0, 192), "3~Observeur 'Core.BialogDox' inconnu{[newline]}Veuillez utiliser le client Java.#CORE/printmodelReinit?#<?xml version='1.0' encoding='ISO-8859-1'?><REPONSES><REPONSE observer='Core.BialogDox' ");
	}
});
...
test( "Authentification", function() {
	...
	this.mHttpTransport.XmlReceved = "<?xml version='1.0' encoding='ISO-8859-1'?><REPONSES><REPONSE observer='CORE.Auth' source_extension='"+AUTH_PARAM_NAME[4]+"' source_action='authentification'><![CDATA[NEEDAUTH]]></REPONSE></REPONSES>";
	ok(!this.mObserverFactory.setAuthentification("abc", "123"),"Bad");
	equal(this.mHttpTransport.getSession(), "", "Session Bad");

	equal(this.mHttpTransport.XmlParam.size(), 3, 'XmlParam size');
	equal(this.mHttpTransport.XmlParam['WebFile'] , AUTH_PARAM_NAME[4]+"/authentification", 'WebFile');
	equal(this.mHttpTransport.XmlParam[AUTH_PARAM_NAME[0]] , "abc", AUTH_PARAM_NAME[0]);
	equal(this.mHttpTransport.XmlParam[AUTH_PARAM_NAME[1]] , "123", AUTH_PARAM_NAME[1]);

	this.mHttpTransport.XmlReceved = "<?xml version='1.0' encoding='ISO-8859-1'?><REPONSES><REPONSE observer='CORE.Auth' source_extension='"+AUTH_PARAM_NAME[4]+"' source_action='authentification'><CONNECTION></CONNECTION><PARAM name='ses' type='str'>admin1176588477</PARAM><![CDATA[OK]]></REPONSE></REPONSES>";
	ok(this.mObserverFactory.setAuthentification("admin", "admin"),"auth OK");
	equal(this.mHttpTransport.getSession(), "admin1176588477", "Session OK");

	equal(this.mHttpTransport.XmlParam.size(), 3, 'XmlParam size');
	equal(this.mHttpTransport.XmlParam['WebFile'] , AUTH_PARAM_NAME[4]+"/authentification", 'WebFile');
	equal(this.mHttpTransport.XmlParam[AUTH_PARAM_NAME[0]] , "admin", AUTH_PARAM_NAME[0]);
	equal(this.mHttpTransport.XmlParam[AUTH_PARAM_NAME[1]] , "admin", AUTH_PARAM_NAME[1]);
});
