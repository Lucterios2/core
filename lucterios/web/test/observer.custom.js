...
test( "Custom_Simple", function() {
	...
	var lbl = jcnt.find("table:eq(0) > tbody > tr:eq(0) > td:eq(0) > span");
	equal(lbl.html(),"<center><b><u>Gestion documentaire</u></b></center>","text 1");

	var lbl = jcnt.find("table:eq(0) > tbody > tr:eq(1) > td:eq(0) > span");
	equal(lbl.html(),"<center>13 fichiers actuellement disponibles</center>","text 2");

	var lbl = jcnt.find("table:eq(0) > tbody > tr:eq(2) > td:eq(0) > span");
	equal(lbl.html(),"<center><i>Taille de stockage: restant 30.03 Mo. sur 1 Go.</i></center>","text 3");

	var lbl = jcnt.find("table:eq(0) > tbody > tr:eq(3) > td:eq(0) > span");
	equal(lbl.html(),"<center><hr></center>","text 4");

	var lbl = jcnt.find("table:eq(0) > tbody > tr:eq(4) > td:eq(0) > span");
	equal(lbl.html(),"<center><b><u>Mises à jour</u></b></center>","text 5");

	var lbl = jcnt.find("table:eq(0) > tbody > tr:eq(5) > td:eq(0) > span");
	equal(lbl.html(),"<center>Votre logiciel est à jour.</center>","text 6");

	var lbl = jcnt.find("table:eq(0) > tbody > tr:eq(6) > td:eq(0) > span");
...
test( "Custom_EditMemo", function() {
	...
	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(0) > td:eq(0) > img");
	equal(comp.attr("src").substr(0,71),"URL_ROOT/extensions/org_lucterios_contacts/images/contactMoral.png?val=","comp 1");
	...
	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(1) > td:eq(0) > span");
	...
	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(2) > td:eq(1) > input");
	equal(comp.val(),"99000","comp 7");
	comp.focus();
	comp.val("123456");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(2) > td:eq(2) > span");
	equal(comp.html(),"<b>Ville</b>","comp 8");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(2) > td:eq(3) > input");
	equal(comp.val(),"TRIFOUILLY","comp 9");
	comp.focus();
	comp.val("DDDD");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(3) > td:eq(1) > span");
	equal(comp.html(),"<b>Pays</b>","comp 10");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(3) > td:eq(2) > input");
	equal(comp.val(),"","comp 11");
	comp.focus();
	comp.val("EEEE");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(4) > td:eq(1) > span");
	equal(comp.html(),"<b>Tel. Fixe</b>","comp 12");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(4) > td:eq(2) > input");
	equal(comp.val(),"0987654321","comp 13");
	comp.focus();
	comp.val("FFFFF");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(4) > td:eq(3) > span");
	equal(comp.html(),"<b>Tel. Portable</b>","comp 14");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(4) > td:eq(4) > input");
	equal(comp.val(),"","comp 15");
	comp.focus();
	comp.val("GGGGG");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(5) > td:eq(1) > span");
	equal(comp.html(),"<b>Fax</b>","comp 16");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(5) > td:eq(2) > input");
	equal(comp.val(),"","comp 17");
	comp.focus();
	comp.val("HHHHH");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(6) > td:eq(1) > span");
	equal(comp.html(),"<b>Courriel</b>","comp 18");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(6) > td:eq(2) > input");
	equal(comp.val(),"info@sd-libre.fr","comp 19");
	comp.focus();
	comp.val("IIIII");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(7) > td:eq(1) > span");
	equal(comp.html(),"<b>Informations légales</b>","comp 20");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(7) > td:eq(2) > textarea");
	equal(comp.text(),"","comp 21");
	comp.focus();
	comp.text("JJ\nKKKK");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(8) > td:eq(1) > span");
	equal(comp.html(),"<b>Commentaire</b>","comp 22");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(8) > td:eq(2) > textarea");
	equal(comp.text(),"","comp 23");
	comp.focus();
	comp.text("LL\nMMMM\nNN");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(9) > td:eq(1) > span");
	equal(comp.html(),"<b>Mot de passe</b>","comp 24");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(9) > td:eq(2) > input");
...
asyncTest( "Custom_Upload", function() {
	...
	var parse=xml_receive.parseXML();
	var obs=new ObserverCustom();
	obs.setSource("org_lucterios_contacts","personneMorale_APAS_AddModify");
	obs.setContent(parse);
	obs.show("Nos coordonnées",FORM_MODAL);

	obs.mCompList.get('uploadlogo').getValue=function(){ 
		if (typeof(Blob) === typeof(Function)) {
			var blob_content = new Blob([content_file],{type: "text/plain;charset=UTF-8"});
			return blob_content;
		}
		else {
	...
	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(1) > td:eq(1) > span");
	equal(comp.html(),'<center><i><font size="-2">Importer de préférence une image JPEG de 100x100 pts.</font></i></center>',"comp 3");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(2) > td:eq(1) > span");
	equal(comp.html(),'<b>Courriel</b>',"comp 4");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(2) > td:eq(2) > a");
	...
	equal(Singleton().Transport().XmlParam['URL'],'usr/TestValidation/FileName.txt');
	...
	setTimeout(function() {
		...
		var expected_cmd="org_lucterios_contacts->personneMorale_APAS_AddModifyAct(";
		expected_cmd+="personneMorale='1',uploadlogo='undefined;"+content_base64+"',"
		expected_cmd+=")";
		equal(current_factory.CallList.get(0),expected_cmd,"Call #1");
	}, 600);
});

test( "Custom_advance", function() {
	...
	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(0) > td:eq(0) > img");
	equal(comp.attr("src").substr(0,59),"URL_ROOT/extensions/org_lucterios_task/images/task.png?val=","comp img");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(2) > td:eq(0) > span");
	equal(comp.html(),"<b>Début</b>","comp 1");

	var comp2 = jcnt.find("table:eq(0) > tbody > tr:eq(2) > td:eq(1) > input");
	equal(comp2.val(),"15/08/2012","comp 2");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(2) > td:eq(2) > span");
	equal(comp.html(),"<b>Responsable</b>","comp 3");

	var comp4 = jcnt.find("table:eq(0) > tbody > tr:eq(2) > td:eq(3) > select");
	equal(comp4.val(),"143","comp 4");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(2) > td:eq(4) > button");
	...
	var comp7 = jcnt.find("table:eq(0) > tbody > tr:eq(3) > td:eq(1) > span > input");
	equal(comp7.val(),"11:30","comp 7");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(3) > td:eq(2) > span");
	equal(comp.html(),"<b>Privé</b>","comp 8");

	var comp9 = jcnt.find("table:eq(0) > tbody > tr:eq(3) > td:eq(3) > input");
	...
	var comp11 = jcnt.find("table:eq(0) > tbody > tr:eq(4) > td:eq(1) > select");
	equal(comp11.val(),"3","comp 11");

	var comp = jcnt.find("table:eq(0) > tbody > tr:eq(4) > td:eq(2) > span");
	equal(comp.html(),"<b>Rappel (Nb de jours)</b>","comp 12");

	var comp13 = jcnt.find("table:eq(0) > tbody > tr:eq(4) > td:eq(3) > span > input");
	...
	var comp15a = jcnt.find("table:eq(0) > tbody > tr:eq(5) > td:eq(1) > input");
	equal(comp15a.val(),"12/07/2008","comp 15a");
	var comp15b = jcnt.find("table:eq(0) > tbody > tr:eq(5) > td:eq(1) > span > input");
	...
	var comp17 = jcnt.find("table:eq(0) > tbody > tr:eq(5) > td:eq(3) > select");
...
test( "Custom_grid", function() {
	...
	var table_comp = jcnt.find("div > div:eq(1) > table:eq(0) > tbody > tr:eq(0) > td:eq(0) > table:eq(0)");
	var table_content = table_comp.find("tbody > tr:eq(0) > td:eq(0)");
	var table_buttons = table_comp.find("tbody > tr:eq(0) > td:eq(1)");

	var comp = table_content.find("table:eq(0) > thead > tr:eq(0) > th:eq(1)");
	equal(comp.html(),"Nom","head 1");

	var comp = table_content.find("table:eq(0) > thead > tr:eq(0) > th:eq(4)");
	equal(comp.html(),"Téléphones","head 2");

	var cell1 = table_content.find("table:eq(0) > tbody > tr:eq(0) > td:eq(2)");
	equal(cell1.html(),"Anne","cell 1");

	var cell2 = table_content.find("table:eq(0) > tbody > tr:eq(1) > td:eq(0) > img:eq(0)");
	equal(cell2.attr("src"),"URL_ROOT/usr/org_lucterios_contacts/Image_101.jpg","cell 2");

	var cell3 = table_content.find("table:eq(0) > tbody > tr:eq(2) > td:eq(4)");
	...
	equal(btn2.prop("disabled"),false,'btn 2 dis');
	equal(row1.hasClass("selected"),false,'row 1:'+row1.attr('class'));
	equal(row2.hasClass("selected"),true,'row 2:'+row2.attr('class'));
	equal(row3.hasClass("selected"),false,'row 3:'+row3.attr('class'));

	row1.click();
	equal(btn1.prop("disabled"),false,'btn 1 dis');
	equal(btn2.prop("disabled"),false,'btn 2 dis');
	equal(row1.hasClass("selected"),true,'row 1:'+row1.attr('class'));
	equal(row2.hasClass("selected"),false,'row 2:'+row2.attr('class'));
	equal(row3.hasClass("selected"),false,'row 3:'+row3.attr('class'));
	...
	equal(row1.hasClass("selected"),false,'row 1:'+row1.attr('class'));
	equal(row2.hasClass("selected"),false,'row 2:'+row2.attr('class'));
	equal(row3.hasClass("selected"),true,'row 3:'+row3.attr('class'));
	...
	equal(this.mObsFactory.CallList.size(),2,"Call D Nb :"+this.mObsFactory.CallList.toString());
});	

test( "Custom_need", function() {
	...
	var parse=xml_receive.parseXML();
	var obs=new ObserverCustom();
	obs.setSource("org_lucterios_contacts","personnePhysique_APAS_AddModify");
	obs.setContent(parse);
	...
	var comp = jcnt.find("table > tbody > tr > td > input[name='prenom']");
	comp.val("Jean");
	var comp = jcnt.find("table > tbody > tr > td > textarea[name='adresse']");
	comp.val("1 rue de la paix");
	var comp = jcnt.find("table > tbody > tr > td > input[name='codePostal']");
	comp.val("12300");
	var comp = jcnt.find("table > tbody > tr > td > input[name='Ville']");
	comp.val("TRUC");
	btn1.click();
	equal(this.mObsFactory.CallList.size(),2,"Call C Nb :"+this.mObsFactory.CallList.toString());
