...
test( "Authentification 1", function() {
	Singleton().Transport().setSession("");
	var xml_receive="<REPONSE observer='CORE.Auth' source_extension='"+AUTH_PARAM_NAME[4]+"' source_action='authentification'>"+
			"<CONNECTION>" +
	...
	equal(description.getCopyRigth(),"General Public Licence - http://www.lucterios.org","CopyRigth");
	equal(description.getLogoIconName(),"http://demo.lucterios.org/Lucterios/extensions/applis/images/logo.gif","LogoIconName");
	equal(description.getInfoServer(),"{[italic]}Apache/2.2.14 (Ubuntu){[/italic]}","InfoServer");
...
test( "Authentification 2", function() {
	...
	equal(this.mObsFactory.CallList.get(0),AUTH_PARAM_NAME[4]+"->authentification("+AUTH_PARAM_NAME[0]+"='machin',"+AUTH_PARAM_NAME[1]+"='coucou',)","Call #1");
});
...
test( "Acknowledge", function() {
	...
	equal(obs.getRedirectAction().getExtension(),"TestValidation","RedirectAction-Extension");
	equal(obs.getRedirectAction().getAction(),"SuperTableTest_APAS_Fiche","RedirectAction-Action");

	equal(this.mObsFactory.CallList.size(),2,"Call Nb");
	equal(this.mObsFactory.CallList.get(0),"CORE->UNLOCK(SuperTableTest='101',)","Call #1");
	equal(this.mObsFactory.CallList.get(1),"TestValidation->SuperTableTest_APAS_Fiche(SuperTableTest='101',)","Call #2");
});
...
test( "Print", function() {
	...
	equal(contents[1].trim(),"\"Liste des Personnes Morales\"","title");
	equal(contents[4].trim(),"\"Raison Sociale\";\"Adresse\";\"Téléphones\";\"Courriel\";","header");
	equal(contents[6].trim(),"\"Le glos minet\";\"Cours Beriat 38000 GRENOBLE FRANCE\";\"04.19.28.37.46\";\"glos.minet@hotmail.fr\";","line #2");
	equal(this.mFileName,'Liste_des_Personnes_Morales.csv','file name');
	equal(this.mObsFactory.CallList.size(),0,"Call Nb");
});
...
test( "Dialog", function() {
	...
	equal(this.mObsFactory.CallList.get(0),"org_lucterios_contacts->personneAbstraite_APAS_Delete(Filtretype='2',ORIGINE='personneMorale_APAS_Fiche',RECORD_ID='100',TABLE_NAME='org_lucterios_contacts_personneMorale',abstractContact='1196',personneMorale='100',CONFIRME='YES',)","Call #1");
	equal(this.mObsFactory.CallList.get(1),"org_lucterios_contacts->personneMorale_APAS_Fiche(Filtretype='2',ORIGINE='personneMorale_APAS_Fiche',RECORD_ID='100',TABLE_NAME='org_lucterios_contacts_personneMorale',abstractContact='1196',personneMorale='100',CONFIRME='YES',)","Call #2");
